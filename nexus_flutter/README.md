...
- **Herdr 双向原生卡片同步**：直连 PC 端 Herdr 分屏，将 TUI 终端映射为原生 Thinking/ToolCall/Markdown 卡片，支持手机向终端注入 Prompt 与中断。
- **两阶段秒开与零抖动列表**：首屏仅渲染当前最新一轮（<5ms 秒开），全量历史由后台静默拉取；采用 `ListView(reverse: true)` 底部绝对锚定，消除任何视口跳动与卡顿。
