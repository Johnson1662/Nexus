...
3. **测试纪律（用户自行测试）**：
   - ⚠️ **禁止动不动就自己开始测试**；
   - 代码修改或构建部署完成后直接交付用户，**所有功能与实机测试由用户自行完成**；
   - 除非用户明确要求（例如指令包含“跑测试”、“验证某个功能”），否则严禁擅自调用 `uitest`、自动化脚本、测试用例或在设备上自发模拟交互进行测试。
...
- **Herdr 会话操作**：空会话不能发送 Native `start`；关闭 `herdr:<paneId>` 必须调用 `pane.close` 并等待 `session_closed` 后再移除本地项；Herdr 会话不提供重命名入口。
