...
set -eu
action="${1:-session}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/nexus-claude-hook.XXXXXX")" || exit 0
...
if action == "stop" or action == "exit" or data.get("hookEventName") == "SessionEnd":
    try:
...
if (action === "stop" || action === "exit" || data.hookEventName === "SessionEnd") {
